...
class AddAllowedIP extends Action
{
    ...
    if($selectModel->allowed_ips !== NULL) {
        $countCurrentIPs = substr_count($selectModel->allowed_ips, ',');
        if($countCurrentIPs > 2) {
            return Action::danger('You have reached max. of allowed IPs, use the \'Truncate IP List\' action on this API key to empty whitelist.');
        }
    }
    ...
    if(filter_var($ip, FILTER_VALIDATE_IP)) {
        ...
        if(str_contains($selectModel->allowed_ips, $ip)) {
            return Action::danger('IP seems to be in whitelist already.');
        }

    if($selectModel->allowed_ips === NULL) {
        $models->first()->update(['allowed_ips' => $ip]);
    } else {
        $models->first()->update(['allowed_ips' => $selectModel->allowed_ips.','.$ip]);
    }
    ...
    public function fields(NovaRequest $request)
    {
        ...
        return [
            ...
            Text::make('IP')->placeholder('123.456.789.100')->rules('required', 'min:5', 'max:66'),
